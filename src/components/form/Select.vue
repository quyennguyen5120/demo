<template>
  <div dropdown tabindex="0" ref="rootRef" class="relative custom-select cursor-pointer" @click="toggleDropdown">
    <div class="rounded-full text-center text-xl" :class="style">
      <div
          class="flex justify-center items-center gap-1 md:gap-2 lg:gap-4 xl:gap-5 text-[0.813rem] md:text-[0.813rem] lg:text-[1.125rem] xl:text-[1.5rem] leading-tight">
        <slot>{{
    ...
    <div class="overflow-y-scroll absolute bg-white left-0 right-0 z-40 mt-2 dropdown-list" id="style-scroll"
         :class="[{ hidden: !state.showDropdown }, options.length >= 10 ? 'h-[31.25rem]' : '']">
      <ul v-if="options.length > 0">
        <li v-for="(item, index) in options" :key="index"
            class="text-center text-[0.813rem] md:text-[0.813rem] lg:text-[1.125rem] xl:text-[1.5rem]"
...
.custom-select .dropdown-list {
  box-shadow: 0px 0px 0.438rem rgba(0, 0, 0, 0.25);
  border-radius: 5px;
}
